...
- 能力：
  ...
  - 版本规划、风险识别与指标定义
- **触发方式**：
  - **命令触发**：`/write-prd [产品类型]`、`/discover [产品方向]`
  - **关键词触发**：`智能家居`、`智能门锁`、`智能照明`、`智能安防`、`传感器`、`通信协议`、`Zigbee`、`Matter`、`芯片选型`、`PRD`、`产品需求文档`
  - **场景触发**：`帮我写PRD`、`设计场景联动`、`分析竞品`、`选协议`、`认证怎么做`、`硬件规格`、`BOM`
- **触发条件**：
  - ✅ 触发场景：智能家居产品规划、功能设计、PRD撰写、技术方案评估、场景设计、竞品分析、认证合规、芯片选型、硬件规格
  - ❌ 非触发场景：通用IoT咨询、纯代码调试、成本精算、销售策略制定、与智能家居无关的技术问题
